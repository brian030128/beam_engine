...
# Fused wrapper compiles against a fixed max_levels (see benchmark_fused_config).
# Configs whose dynamic num_levels exceeds this cap skip the Fused arm.
FUSED_MAX_LEVELS = 64
...
def run_benchmark(
    ...
    for width in widths:
        for depth in depths:
            for topo_name, builder_fn in [
                ...
                # Skip configs whose dynamic num_levels exceeds the fused
                # kernel's compiled cap. multi_meta is the binding factor;
                # two_meta is always num_levels=2.
                if multi_meta["num_levels"] > FUSED_MAX_LEVELS:
                    done += 1
                    print(f"  [{done}/{total_configs}]  W={width:>2d}  D={depth:>3d}  "
                          f"topo={topo_name:<16s}  skip (levels={multi_meta['num_levels']} > {FUSED_MAX_LEVELS})")
                    continue

                multi_ms = benchmark_fused_config(
                    multi_meta, q, kv_data,
                    num_qo_heads, num_kv_heads, head_dim, page_size, dtype,
                    warmup, repeat,
                )
                two_ms = benchmark_fused_config(
                    two_meta, q, kv_data,
                    num_qo_heads, num_kv_heads, head_dim, page_size, dtype,
                    warmup, repeat,
                )
                multi_ref_ms = benchmark_one_config(
                    multi_meta, q, kv_data,
                    num_qo_heads, num_kv_heads, head_dim, page_size, dtype,
                    warmup, repeat,
                )
                two_ref_ms = benchmark_one_config(
                    two_meta, q, kv_data,
                    num_qo_heads, num_kv_heads, head_dim, page_size, dtype,
                    warmup, repeat,
                )
                ...
                results.append((depth, width, topo_name, multi_ms, two_ms,
                                multi_ref_ms, two_ref_ms,
                                speedup, multi_meta["num_levels"]))
                done += 1
                m_vs_ref = multi_ms / multi_ref_ms if multi_ref_ms > 0 else float("inf")
                t_vs_ref = two_ms / two_ref_ms if two_ref_ms > 0 else float("inf")
                print(f"  [{done}/{total_configs}]  W={width:>2d}  D={depth:>3d}  "
                      f"topo={topo_name:<16s}  "
                      f"multi={multi_ms:.4f}ms (ref={multi_ref_ms:.4f}, F/R={m_vs_ref:.2f}x)  "
                      f"two={two_ms:.4f}ms (ref={two_ref_ms:.4f}, F/R={t_vs_ref:.2f}x)  "
                      f"M/T={speedup:.2f}x  "
                      f"(levels={multi_meta['num_levels']}, leaves={total_q})")

            # Tree-Sampled topology: Monte Carlo with N random SD-style samples.
            # Multi (fused) only contributes when num_levels ≤ FUSED_MAX_LEVELS;
            # Two (fused) and the non-fused refs always contribute.
            rand_multi_total = 0.0
            rand_multi_count = 0
            rand_two_total = 0.0
            rand_two_count = 0
            rand_multi_ref_total = 0.0
            rand_multi_ref_count = 0
            rand_two_ref_total = 0.0
            rand_two_ref_count = 0
            for sample_i in range(random_n):
                ...
                if multi_meta["num_levels"] <= FUSED_MAX_LEVELS:
                    rand_multi_total += benchmark_fused_config(
                        multi_meta, q, kv_data,
                        num_qo_heads, num_kv_heads, head_dim, page_size, dtype,
                        warmup, repeat,
                    )
                    rand_multi_count += 1
                rand_two_total += benchmark_fused_config(
                    two_meta, q, kv_data,
                    num_qo_heads, num_kv_heads, head_dim, page_size, dtype,
                    warmup, repeat,
                )
                rand_two_count += 1
                rand_multi_ref_total += benchmark_one_config(
                    multi_meta, q, kv_data,
                    num_qo_heads, num_kv_heads, head_dim, page_size, dtype,
                    warmup, repeat,
                )
                rand_multi_ref_count += 1
                rand_two_ref_total += benchmark_one_config(
                    two_meta, q, kv_data,
                    num_qo_heads, num_kv_heads, head_dim, page_size, dtype,
                    warmup, repeat,
                )
                rand_two_ref_count += 1

            rand_multi = (rand_multi_total / rand_multi_count
                          if rand_multi_count > 0 else float("nan"))
            rand_two = (rand_two_total / rand_two_count
                        if rand_two_count > 0 else float("nan"))
            rand_multi_ref = (rand_multi_ref_total / rand_multi_ref_count
                              if rand_multi_ref_count > 0 else float("nan"))
            rand_two_ref = (rand_two_ref_total / rand_two_ref_count
                            if rand_two_ref_count > 0 else float("nan"))
            rand_spd = (rand_multi / rand_two
                        if (math.isfinite(rand_multi) and rand_two > 0) else float("nan"))
            results.append((depth, width, "Tree Sampled", rand_multi, rand_two,
                            rand_multi_ref, rand_two_ref, rand_spd, 0))
            done += 1
            multi_str = (f"{rand_multi:.4f}ms ({rand_multi_count}/{random_n})"
                         if math.isfinite(rand_multi) else "    skip (>cap)")
            mr_str = (f"{rand_multi/rand_multi_ref:.2f}x"
                      if math.isfinite(rand_multi) and rand_multi_ref > 0 else " -- ")
            tr_str = (f"{rand_two/rand_two_ref:.2f}x"
                      if rand_two_ref > 0 else " -- ")
            mt_str = f"{rand_spd:.2f}x" if math.isfinite(rand_spd) else "  -- "
            print(f"  [{done}/{total_configs}]  W={width:>2d}  D={depth:>3d}  "
                  f"topo={'Tree Sampled':<16s}  "
                  f"multi={multi_str} (ref={rand_multi_ref:.4f}, F/R={mr_str})  "
                  f"two={rand_two:.4f}ms (ref={rand_two_ref:.4f}, F/R={tr_str})  "
                  f"M/T={mt_str}  (N={random_n})")
    ...
    print(f"  {'W':>3}  {'D':>3}  {'Topo':>16}  "
          f"{'Multi-F':>9}  {'Multi-R':>9}  {'M F/R':>6}  "
          f"{'Two-F':>9}  {'Two-R':>9}  {'T F/R':>6}  {'M/T':>6}")
    print(f"  {'-'*3}  {'-'*3}  {'-'*16}  "
          f"{'-'*9}  {'-'*9}  {'-'*6}  {'-'*9}  {'-'*9}  {'-'*6}  {'-'*6}")
    for depth, width, topo, multi, two, multi_ref, two_ref, spd, lvls in results:
        multi_cell = f"{multi:>9.4f}" if math.isfinite(multi) else f"{'skip':>9s}"
        spd_cell = f"{spd:>5.2f}x" if math.isfinite(spd) else f"{'--':>6s}"
        if math.isfinite(multi) and multi_ref > 0:
            mr_cell = f"{multi/multi_ref:>5.2f}x"
        else:
            mr_cell = f"{'--':>6s}"
        tr_cell = (f"{two/two_ref:>5.2f}x"
                   if two_ref > 0 else f"{'--':>6s}")
        print(f"  {width:>3}  {depth:>3}  {topo:>16}  "
              f"{multi_cell}  {multi_ref:>9.4f}  {mr_cell}  "
              f"{two:>9.4f}  {two_ref:>9.4f}  {tr_cell}  {spd_cell}")
    print(f"{'='*120}")
    ...
    if do_plot:
        try:
            ...
            for topo_name in topo_names:
                ...
                for d, w, tp, _, _, _, _, spd, _ in results:
                    if tp != topo_name:
                        continue
