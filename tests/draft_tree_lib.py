...
def benchmark_fused_config(
    ...
    # Pick force_cta_tile_q from workload structure. The wrapper's adaptive
    # router picks T_lg ∈ {64, 128} by pad-fraction analysis but has three
    # pathologies for cascade SD draft trees:
    #
    #   (A) Few large-R groups in a *short-kv* level (lollipop stem).
    #       Adaptive picks T=128 (lowest pad), gives <num_sm CTAs, and the
    #       short-kv level can't recover via split-K. Forcing T=64 doubles
    #       CTA count and collapses to a single pool_64 launch.
    #
    #   (B) Mid-range packed (≥64) at a *long-kv* level (e.g. prompt-
    #       attending L0 with W ≥ 16). pad_frac at T=128 reaches 50%+ and
    #       adaptive_force_t16 fires, routing the long-kv scan through
    #       pool_16 — which loads kv 4× compared to T=64 (bandwidth-bound).
    #       Forcing T=64 keeps the long-kv scan in pool_64 with full MMA
    #       util.
    #
    #   (C) Mixed large-R + many small-R tiles (e.g. lollipop with a wide
    #       leaf burst). Adaptive picks T=64 for the large-R level but
    #       spawns a separate pool_16 launch for the leaves. With many
    #       small-R tiles the second launch is non-trivial; collapsing
    #       both into pool_64 saves the launch + scheduling overhead even
    #       though leaf tiles get padded to T=64.
    #
    # When none of these apply, leaving force=None lets adaptive use 2
    # pools — the right call for pure-small-R workloads (W=4 trees) where
    # pool_16 wins, or for very wide trees with packed ≫ 64 where T=128
    # is genuinely best.
    MIN_KV_LEN_FOR_SPLIT = 1024
    MERGE_POOLS_SMALL_R_COUNT = 16    # threshold for collapsing pool_lg + pool_16
    gqa = num_qo_heads // num_kv_heads
    num_sm = torch.cuda.get_device_properties(q.device).multi_processor_count
    target_large_tiles = max(num_sm // num_kv_heads, 1)
    has_underutil_large = False
    has_long_kv_level = False
    small_r_count = 0
    has_large_r = False
    real_a = padded_a = 0
    for lvl in range(num_levels):
        qo_h = meta["qo_indptr_arr"][lvl].cpu().tolist()
        kv_lens = kv_len_arr[lvl].cpu().tolist()
        large_groups = 0
        max_kv_len = 0
        for i in range(len(qo_h) - 1):
            packed = (qo_h[i + 1] - qo_h[i]) * gqa
            kv_l = kv_lens[i] if i < len(kv_lens) else 0
            if packed <= 16:
                small_r_count += 1
            else:
                large_groups += 1
                has_large_r = True
                # Mirror wrapper's _predict_pad_at_T(128) for adaptive_force_t16.
                if packed <= 128:
                    real_a += packed
                    padded_a += 128
                else:
            ...
            if kv_l > max_kv_len:
                max_kv_len = kv_l
        if max_kv_len >= MIN_KV_LEN_FOR_SPLIT:
            has_long_kv_level = True
        # (A) under-utilized large groups in a short-kv level.
        if (
            0 < large_groups < target_large_tiles
            and max_kv_len < MIN_KV_LEN_FOR_SPLIT
        ):
            has_underutil_large = True
    pad_frac_a = (1.0 - real_a / padded_a) if padded_a > 0 else 0.0
    has_small_r = small_r_count > 0
    # (B) adaptive would force T=16 (pad-heavy at T=128) and we have a
    # long-kv level that would suffer 4× kv bandwidth at T=16. Only fires
    # at pad_frac_a ≥ 0.5 (max_packed ≈ 64, perfect T=64 fit). For
    # pad_frac in (0.4, 0.5) the T=16 path's higher parallelism wins.
    would_force_t16_with_long_kv = (
        pad_frac_a >= 0.5 and has_long_kv_level
    )
    # (C) mixed large-R + many small-R workload (e.g. lollipop with wide
    # leaf burst). Adaptive picks T=64 already (good for the large-R
    # level) but spawns a separate pool_16 launch for the small-R tiles.
    # When the small-R count is large, the extra launch overhead and the
    # split scheduling outweigh the per-tile padding cost of merging into
    # pool_64. Force=64 to collapse to a single pool launch.
    merge_pools = (
        has_large_r
        and small_r_count >= MERGE_POOLS_SMALL_R_COUNT
    )
    force_cta = (
        64
        if (
            (has_small_r and has_underutil_large)
            or would_force_t16_with_long_kv
            or merge_pools
        )
        else None
    )
