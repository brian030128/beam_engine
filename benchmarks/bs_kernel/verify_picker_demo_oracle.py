...
# (cell, K, B, mn, L_p_hint, L_tail_hint, num_qo, num_kv, bpkv, kv_dtype, oracle)
...
# kv_dtype is the canonical label the cost model uses to look up per-dtype
# calibrated rates (see calibrate.py / cost_model.canonical_kv_dtype).
...
CELLS = [
    ...
    ("A2_mcr_K4_B1",      4,  1,  32,  500,   16, 32,  8, 2048, "bf16",     "1POOL"),
    ("A3_mdq_K32_B1",    32,  1, 256, 40000, 128, 32,  8, 2048, "bf16",     "1POOL"),
    ("A4_mfs_K16_B8",    16,  8, 256,  6500,  80, 32,  8, 2048, "bf16",     "1POOL"),
    ("A5_mls_K32_B4",    32,  4, 256,  4000,  80, 32,  8, 2048, "bf16",     "1POOL"),
    # 8B (Llama-3.1-8B-FP8, bf16 KV, num_qo=32, num_kv=8, hd=128)
    ("B2_mcr_K4_B1",      4,  1,  32,  500,   16, 32,  8, 4096, "bf16",     "1POOL"),
    ("B3_mdq_K64_B1",    64,  1, 256, 40000, 128, 32,  8, 4096, "bf16",     "DT"),
    ("B4_mfs_K32_B4",    32,  4, 256,  6500,  80, 32,  8, 4096, "bf16",     "1POOL"),
    ("B5_mls_K64_B4",    64,  4, 256,  4000,  80, 32,  8, 4096, "bf16",     "1POOL"),
    # 70B (Llama-3-70B-FP8, fp8 KV, TP=2: num_qo=32, num_kv=4, hd=128)
    ("C3_mdq_K32_B1",    32,  1, 256, 86000, 150, 32,  4, 1024, "fp8_e4m3", "DT"),
    ("C4_mfs_K16_B4",    16,  4, 256,  6500,  80, 32,  4, 1024, "fp8_e4m3", "1POOL"),
    ("C5_mls_K32_B4",    32,  4, 256,  4000,  80, 32,  4, 1024, "fp8_e4m3", "1POOL"),
    ("C6_mcr_K64_B4",    64,  4, 256,  4000,  80, 32,  4, 1024, "fp8_e4m3", "DT"),
]
...
def main():
    ...
    print(f"{'cell':<22} {'kv_dt':>9} {'oracle':>7} {'picked':>22} {'match':>6}")
    print("-" * 72)
    correct = 0
    total = 0
    for (cell, K, B, mn, Lp, Ltail, nqo, nkv, bpkv, kv_dtype, oracle) in CELLS:
        w = WorkloadShape(
        ...
        if picked_class == oracle:
            correct += 1
        total += 1
        print(f"{cell:<22} {kv_dtype:>9} {oracle:>7} {picked:>22} {match:>6}")
